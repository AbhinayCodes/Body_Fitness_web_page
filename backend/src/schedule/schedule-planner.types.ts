...
export interface DailyScheduleInput { wakeTime: string; sleepTime: string; gymTime?: string; workoutDurationMinutes: number; isTrainingDay: boolean; dietType: string; restrictions: string[]; targets: NutritionTargets; }
export interface ScheduledMeal { slot: string; scheduledMinutes: number; targetCalories: number; recipeId: string; alternativeRecipeIds: string[]; }
